  ...
  >
      ...
      <div class="flex items-center justify-between h-16 relative">
        ...
        <div class="flex items-center space-x-6">
          <NuxtLink to="/" class="mr-2">
            <span
              class="text-yellow-500 px-2 py-1 rounded font-extrabold text-xl tracking-wide"
              >NUFLIX</span
            >
          </NuxtLink>
          <nav class="hidden md:flex items-center space-x-5 text-sm">
            <NuxtLink
              to="/films"
              exact-active-class="font-bold text-yellow-400"
            ...
            <NuxtLink
              to="/tv-shows"
              exact-active-class="font-bold text-yellow-400"
              class="hover:text-yellow-400 flex items-center gap-1"
            >
        ...
        <div
          class="absolute left-1/2 transform -translate-x-1/2 w-full max-w-lg px-4 hidden md:block"
        >
          <div class="relative">
            <div
              class="relative flex bg-white rounded overflow-hidden shadow-md"
            >
              <!-- Search Input -->
            ...
            >
              ...
              <div
                v-else-if="resultsDropdown.length === 0"
              ...
              <div
                v-else
                class="grid grid-cols-1 gap-2 p-2 max-h-[700px] overflow-y-auto"
              >
                <NuxtLink
                  ...
                  <img
                    :src="
                      item.poster_path || item.profile_path
                        ? `${config.public.imageBaseUrl}/w200${item.poster_path || item.profile_path}`
              ...
              <p class="text-gray-300 px-2 py-4">
                See all search results for "<span
        ...
        <!-- RIGHT SIDE -->
        <div
          class="flex items-center justify-between space-x-4 md:space-x-4 px-4"
        >
          <button class="md:hidden" @click="toggleMobileSearch">
            <Icon name="mdi:magnify" size="1.5em" />
          </button>
          <NuxtLink
            to="/watchlist"
            class="hidden md:flex items-center text-sm hover:text-yellow-400"
            exact-active-class="font-bold text-yellow-400"
          >
          ...
          <button
            ...
            class="w-8 h-8 flex items-center justify-center rounded border border-gray-500 hover:bg-gray-700 transition"
            @click="toggleTheme"
          >
            <Icon
    ...
    <Transition name="fade-slide">
      <div
        ...
        class="md:hidden px-4 py-3 bg-[#121212] shadow-sm space-y-2 relative"
      >
    ...
    <Transition name="fade-slide">
      <nav
        v-if="isMobileMenuOpen"
        class="md:hidden bg-opacity-95 text-white text-center px-6 py-4 space-y-4"
      >
        <NuxtLink
          to="/films"
          exact-active-class="font-bold text-yellow-400"
...
const performSearch = () => {
  ...
  if (query) {
    navigateTo(
      `/search?query=${encodeURIComponent(query)}&filter=${selectedFilter.value.key}`
    )
