<template>
  <div
    class="relative group w-full max-w-[210px] mx-auto rounded-lg overflow-hidden bg-black/80 shadow-md border border-black/20 transition duration-300"
  >
    <div
      class="absolute inset-0 bg-black/20 group-hover:bg-black/50 transition duration-300 pointer-events-none z-0"
    ></div>
    <div
      class="absolute -top-2 -left-3 w-full flex items-center justify-between"
    >
      <!-- Watchlist Butonu -->
      <div
    ...
    <div class="p-4">
      <!-- Fragman Butonu -->
      <div
        class="absolute top-1/2 transform -translate-y-1/2 mt-10 left-2 flex items-center justify-center"
      >
        <button
        ...
        >
          <Icon
            name="mdi:play"
            class="text-2xl text-white font-bold hover:text-yellow-500"
          />
        </button>
      ...
      <!-- Başlık -->
      <p
        class="font-semibold text-white text-lg hover:underline cursor-pointer"
        @click.stop="navigateTo(`/tv/${tv.id}`)"
      >
        {{ tv.name }}
      </p>
