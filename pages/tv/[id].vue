...
onMounted(async () => {
  ...
  try {
    ...
    creatorName.value =
      credits.crew.find(
        (c: any) => c.job === 'Creator' || c.job === 'Executive Producer'
      )?.name || 'Unknown'
...
const isFavorite = computed(() =>
  favoritesStore.favorites.includes(tv.value?.id)
)
const inWatchlist = computed(() =>
  watchlistStore.movies.some(item => item.id === tv.value?.id)
)
...
<template>
  <div>
    ...
    <section
      v-else
      class="relative text-white overflow-hidden px-4 py-8 bg-center bg-cover"
    >
      <!-- Blurred backdrop -->
      <div
        class="absolute inset-0 z-10 scale-105 bg-cover bg-center blur-sm opacity-25"
        :style="{
          backgroundImage: `url(${config.public.imageBaseUrl}/original${tv.backdrop_path})`,
        }"
      />
        ...
        <div class="md:col-span-2 flex flex-col gap-4">
          <h1 class="text-4xl font-bold leading-tight">
            ...
            <span class="text-2xl text-gray-400"
              >({{ tv.first_air_date?.slice(0, 4) }})</span
            >
          </h1>
          ...
          <div class="flex items-center gap-4 mb-6">
            ...
            <!-- User vibe / rating -->
            <button
              class="flex items-center gap-2 text-sm border border-white/40 rounded-xl px-4 py-1 bg-white/10"
              @click="showVibeModal = true"
            >
              <span class="font-semibold">{{
                ratingUser ? 'Your Vibe' : "What's your Vibe?"
              }}</span>
          ...
          <p v-if="tv.tagline" class="italic text-lg text-gray-300">
            {{ tv.tagline }}
          </p>
          ...
          >
            <div>
              <p>
                <strong class="text-gray-200">Durum:</strong>
                <span class="ml-1">{{ tv.status }}</span>
              </p>
              <p>
                <strong class="text-gray-200">Yaratıcı:</strong>
                <span class="ml-1">{{ creatorName }}</span>
              </p>
              <p>
                <strong class="text-gray-200">Diller:</strong>
                <span class="ml-1">{{
                  tv.spoken_languages.map((l: any) => l.name).join(', ')
                }}</span>
              </p>
            </div>
            <div>
              <p>
                <strong class="text-gray-200">Bölüm Sayısı:</strong>
                <span class="ml-1">{{ tv.number_of_episodes }}</span>
              </p>
              <p>
                <strong class="text-gray-200">Ülkeler:</strong>
                <span class="ml-1">{{
                  tv.production_countries.map((c: any) => c.name).join(', ')
                }}</span>
              </p>
              <p v-if="tv.homepage">
                <strong class="text-gray-200">Website:</strong>
                <a
                  :href="tv.homepage"
                  target="_blank"
                  rel="noopener"
                  class="underline ml-1 text-blue-400"
                  >Ziyaret Et</a
                >
              </p>
            </div>
      ...
      <div class="overflow-x-auto whitespace-nowrap scroll-on-hover">
        <div class="inline-flex space-x-4">
          ...
          >
            ...
            />
            <h3 class="text-gray-200 text-sm font-semibold mt-1 truncate">
              {{ cast.name }}
            </h3>
            <p class="text-gray-400 text-xs italic truncate">
              as {{ cast.character }}
            </p>
          </div>
        </div>
